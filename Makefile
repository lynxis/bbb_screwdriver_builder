...
.stamp-compiled: .stamp-prepared
	$(UMASK); \
	  $(MAKE) -C openwrt IGNORE_ERRORS=m $(MAKE_ARGS)
	touch $@
...
.stamp-firmwares: .stamp-compiled
	...
	$(eval IB_FILE := $(shell ls $(OPENWRT_DIR)/bin/$(MAINTARGET)/*-ImageBuilder*$(TARGET)*.tar.bz2))
	...
	cp -a $(OPENWRT_DIR)/bin/$(MAINTARGET)/*ImageBuilder*.tar.bz2 $(FW_TARGET_DIR)/
	# copy packages
